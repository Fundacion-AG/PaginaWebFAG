...
if (! class_exists("modYTNewsBasicHelper") ) { 
require_once (dirname(__FILE__) .DS. 'assets' .DS.'ytcontent.php');

class modYTNewsBasicHelper {
	var $module_name = '';
	function process($params, $module) {
		...
		if($enable_cache==1) {		
			...
			$items =  $cache->get( array('modYTNewsBasicHelper', 'getList'), array($params, $module));
		} else {
			$items = modYTNewsBasicHelper::getList($params, $module);
		}
